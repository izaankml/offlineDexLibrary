...
The two `copyTo`s and the ~75 dimension calls dominate; everything else is minor.

---

## 3. Upload flow

### 3.1 Drop the clear step and the marker column *(small change, large saving)*
...
- **Robustness gain:** there is no longer a column that must sit "just past the block"
...
menu item goes away. CF fills also take precedence over the creator's own CF if placed first.
...
- **Read** the source formats cross-spreadsheet in one call:
  ...
  which gives formats, formulas, row heights/hidden, column widths/hidden.
...
maps `exclude`/`increment` names to columns, and **throws** naming the header it found
...
Also: `nudgeFinishSetupIfFresh` and the update CLI could run the probe as a
...
- The dialog: keep the creator's HTML pristine and ship your own `OfflineDexUpload.html`
  (opened by your menu items), or keep the 2-line edit; either way it's the only edit.
...
Given your TypeScript preference, the second is the better long-term structure; the first
...
|---|---|---|---|---|
| 0 | Timing log (§6.3) | 1 h | none | measures everything below |
...
| 2 | Header-keyed layout probe used by tracker + migrator, with preflight (§5, §4.3) | 1 day | none | catches creator reshuffles before painting/migrating |
| 3 | Surface errors; don't mark migrated on ERR (§4.5, §3.6) | 1 h | none | no more silent mis-migrations |
...
| 6 | Bound-surface shrink (§6.1) | ½ day | none | near-conflict-free merges |
| 7 | Tests / TS bundle / shared naming (§6.2) | 1–2 days | none | long-term maintainability |
| 8 | Prototype CF live diff on Quick Checklist (§3.5) | ½ day | if it holds up: upload ≈ 6 calls | live, always-correct highlights |
| 9 | Spec-driven customizations (§4.2) | 1–2 days | none | no dependency on the previous copy |
