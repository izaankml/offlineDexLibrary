# OfflineDex Scripts

Automation on top of the OfflineDex spreadsheet: save-change highlighting, per-version migration, and a three-step update flow for new creator releases. See `CONTEXT.md` for full design notes and `UPDATING.md` for the update runbook.

## Repo layout

```
├── src/
│   ├── lib/          OfflineDex Library sources (TypeScript) → bundled to library/Code.js
│   │   ├── index.ts        the public surface (each export = an OfflineDexLib.<fn>)
│   │   ├── saveTracker.ts  snapshot / highlight / clear
│   │   ├── migrator.ts     migration plan → preview → atomic apply
│   │   ├── setup.ts        Prepare Next Version / Finish Setup
│   │   └── progress.ts     toasts + the `_timings` log
│   └── shared/naming.ts    "Offline RogueDex X.YY" rules, shared with the CLI
├── library/          OfflineDex Library: standalone Apps Script project
│   ├── appsscript.json
│   └── Code.js           GENERATED by `npm run build` (gitignored)
├── bound/            Bound script: per-version, lives inside each spreadsheet copy (plain JS,
│   ├── appsscript.json     because it is 3-way merged with the creator's code)
│   ├── OfflineDexBound.js    ours: menu, wrappers, uploadFileTracked
│   ├── OfflineDexUpload.html ours: the upload dialog
│   ├── onOpen.js             creator's + one offlineDexOnOpen() call
│   ├── LoadPlayerData.js     creator's, pristine
│   └── UploadPlayerData.html creator's, pristine
├── scripts/
│   ├── build.ts      `npm run build`: esbuild bundle of src/lib into library/Code.js
│   └── update.ts     `npm run update`: the terminal half of a version update
├── test/             node:test suite with an in-memory fake SpreadsheetApp (`npm test`)
├── hooks/pre-push    build + clasp push on git push
├── UPDATING.md       Per-version runbook
├── CONTEXT.md        Full design and decision log
├── IMPROVEMENTS.md   2026-08 efficiency/structure review and the plan being executed
└── README.md         This file
```

`.clasp.json` files are gitignored (contain Script IDs). Each project directory needs one locally; see the formats below.

---

## First-time setup (after cloning)

```bash
npm install        # typescript, esbuild, types
npm run setup      # installs the git hooks (git push → build + clasp push)
npm run check      # typecheck + tests + build
```

---

## Prerequisites

- Node.js v24+ (the update CLI is TypeScript run directly via Node's type stripping)
- clasp: `npm install -g @google/clasp`, then `clasp login` (one-time OAuth flow)
- Apps Script API enabled: https://script.google.com/home/usersettings

---

## One-time library setup

Do this once. The library project is stable across all spreadsheet versions.

**1. Create the Apps Script project**

Go to [script.google.com](https://script.google.com), create a new standalone project named "OfflineDex Library".

**2. Wire up clasp**

```bash
cd library
```

Create `library/.clasp.json` with the library's Script ID (from Project Settings > Script ID in the Apps Script editor):

```json
{
  "scriptId": "YOUR_LIBRARY_SCRIPT_ID",
  "rootDir": "."
}
```

**3. Build and push the library**

```bash
npm run build      # writes library/Code.js from src/lib
cd library && clasp push
```

**4. Deploy as a library**

In the Apps Script editor: Deploy → New deployment → type: Library → add a description → Deploy.

Note the deployment version number. You'll need this in the bound script's `appsscript.json`.

**5. Fill in the manifest placeholder**

In `bound/appsscript.json`, replace `FILL_IN_LIBRARY_SCRIPT_ID` with the library's Script ID and confirm the `version` number matches what you deployed.

---

## One-time bound manifest setup

The bound `appsscript.json` needs two libraries: your `OfflineDexLib` (above) and the creator's `cCryptoGS`. You need to get `cCryptoGS`'s Script ID from the creator's project.

**How to get the cCryptoGS Script ID:**

1. Open any existing OfflineDex spreadsheet copy (one you've already been using)
2. Extensions → Apps Script → Libraries (left sidebar)
3. Click `cCryptoGS` → copy the Script ID and note the version

Fill both into `bound/appsscript.json`:

```json
{
  "timeZone": "America/Los_Angeles",
  "dependencies": {
    "libraries": [
      {
        "userSymbol": "OfflineDexLib",
        "libraryId": "YOUR_LIBRARY_SCRIPT_ID",
        "version": "YOUR_LIBRARY_VERSION",
        "developmentMode": true
      },
      {
        "userSymbol": "cCryptoGS",
        "libraryId": "ACTUAL_CCRYPTOGS_SCRIPT_ID",
        "version": "CCRYPTOGS_VERSION",
        "developmentMode": false
      }
    ]
  },
  "exceptionLogging": "STACKDRIVER",
  "runtimeVersion": "V8"
}
```

Commit this filled-in `appsscript.json` to git. You only need to do this once. On each
subsequent version update, `npm run update` 3-way merges the creator's manifest into
yours, so your library dependencies are preserved automatically.

---

## Per-version update (e.g., `<old> → <new>`)

Three steps; see **[UPDATING.md](UPDATING.md)**:

1. Old sheet: **RogueDex Functions → Prepare Next Version** (copies the public sheet,
   shows you the command to run).
2. Terminal: `npm run update -- <scriptId>` (merges creator code with yours, pushes).
3. New sheet: **RogueDex Functions → Finish Setup** (migrates, then opens the upload
   dialog).

---

## Updating the library (when you change anything under src/)

```bash
# edit src/lib/*.ts
npm run check                     # typecheck + tests + build
cd library && clasp push          # or just `git push`; the pre-push hook builds and pushes
```

The library is TypeScript bundled by esbuild into one `library/Code.js`; every export of
`src/lib/index.ts` becomes a top-level function the bound script can call as
`OfflineDexLib.<name>`. Tests (`npm test`) run the library against an in-memory fake
`SpreadsheetApp` shaped like the real 6.03 sheets (`test/fixtures/`), so most changes can
be verified without touching a spreadsheet.

The bound manifest references the library with `developmentMode: true`, so every
spreadsheet copy automatically runs the latest pushed code. **No redeployment or version
bump is required.**

---

## Timezone note

Both `appsscript.json` files use `America/Los_Angeles`. Change it if needed; it affects how Apps Script formats dates in logs.
