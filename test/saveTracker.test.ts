...
test('golden: header-keyed layout reproduces the old index-based mappings on 6.03', () => {
  const golden = JSON.parse(readFileSync(join(HERE, 'fixtures', 'golden-mappings.json'), 'utf8')) as Record<
    string,
    { dataFirstRow: number; displayFirstRow: number; cells: [number, number, string][] }
  >
  const ss = buildWorkbook({ rows: 3 })
  for (const spec of TRACKER_SPECS) {
    const r = resolveTracker(spec, ss.getSheetByName(spec.dataSheet)! as never, ss.getSheetByName(spec.displaySheet)! as never)
...
test('diffBlocks: only tracked, non-excluded columns paint; width is the display block', () => {
  ...
  assert.equal(d.backgrounds[0]![14 - r.minDisplayCol], INCREMENT_HIGHLIGHT_COLOR)
...
test('snapshot → change → highlight paints the right display cells and nothing else', () => {
  ...
  setActiveSpreadsheet(ss)
  snapshot()
  for (const t of TRACKER_SPECS) assert.ok(ss.getSheetByName(snapshotSheetName(t.key))!.hidden)
  ...
  assert.equal(qDisp.backgroundAt(17, Q.displayRibbonsCol), null, 'Ribbons is outside the tracked block')
  ...
  assert.ok(!calls.some((c) => /Quick Checklist\.setBackgrounds\(\d+,[1-7],/.test(c)), 'no writes into A–G')
})
...
test('processChanges: full flow; a no-change upload clears old highlights and costs few calls', () => {
  ...
  resetToastProgress('upload')
  processChanges()
  assert.equal(ss.getSheetByName('Quick Checklist')!.backgroundAt(12, 8), null, 'previous highlight cleared by the paint')
  assert.ok(logs.some((l) => l.includes('QuickChecklist: highlighted 0 changed cells')))
  const sheetCalls = calls.filter((c) => !c.startsWith('toast') && !c.startsWith('_timings') && !c.startsWith('getSheetByName'))
  console.log(`      [info] Sheets calls for one no-change upload (20 rows): ${sheetCalls.length} (+${calls.length - sheetCalls.length} toasts/timings/lookups)`)
  assert.ok(sheetCalls.length <= 45, `expected ≤45 sheet calls, got ${sheetCalls.length}:\n${sheetCalls.join('\n')}`)
  assert.equal(calls.filter((c) => c.includes('.sort(')).length, 1, 'only the Form Checklist sort; displays were in order')
})
...
test('upgrade: a v1 snapshot (header-offset rows) diffs correctly once, then v2 is written', () => {
  ...
  processChanges()
  const q = ss.getSheetByName('Quick Checklist')!
  assert.equal(q.backgroundAt(14, 8), QUICK_CHECKLIST_HIGHLIGHT_COLOR, 'diffed against the v1 rows')
  assert.equal(q.backgroundAt(12, 8), null)
  ...
  assert.equal(ss.getSheetByName(snapshotSheetName('QuickChecklist'))!.valueAt(14, 4), 42)
...
test('a creator layout change stops the paint with a precise message', () => {
  ...
  resetToastProgress('upload')
  assert.throws(() => processChanges(), /could not find the header "Fought Flag" .* "Starter Dex Checklist"/)
