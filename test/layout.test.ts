...
import { columnLetter, describeResolved, findLabel, resolveFromBands } from '../src/lib/layout.ts'
...
test('Quick Checklist resolves on the migrated 6.03 header (labels replaced by stats)', () => {
  const r = resolveFromBands(spec('QuickChecklist'), HEADERS['STARTER_CHECKLIST.data']!, HEADERS['Quick Checklist (migrated)']!)
...
test('Quick Checklist resolves identically on the fresh PUBLIC 6.03 header', () => {
  const r = resolveFromBands(spec('QuickChecklist'), HEADERS['STARTER_CHECKLIST.data']!, HEADERS['Quick Checklist (fresh PUBLIC)']!)
  assert.equal(r.shift, 4)
  assert.equal(r.minDisplayCol, 8)
})

test('Quick Checklist on the 6.01 layout (no junk column E) resolves to shift +3', () => {
  const display601 = HEADERS['Quick Checklist (fresh PUBLIC)']!.map((row) => [...row.slice(0, 4), ...row.slice(5)])
  const r = resolveFromBands(spec('QuickChecklist'), HEADERS['STARTER_CHECKLIST.data']!, display601)
...
test('dex sheets resolve to the known 6.03 shifts with named exclude/increment columns', () => {
  const s = resolveFromBands(spec('StarterDex'), HEADERS['STARTER_DEX.data']!, HEADERS['Starter Dex Checklist']!)
  ...
  const byDisplay = Object.fromEntries(s.cells.map((c) => [c.displayCol, c.color]))
  ...
  const f = resolveFromBands(spec('FullDex'), HEADERS['FULL_DEX.data']!, HEADERS['Full Dex Checklist']!)
...
test('a creator column insert in the display is absorbed; a renamed anchor fails loudly', () => {
  const display = HEADERS['Starter Dex Checklist']!.map((row) => [...row.slice(0, 3), 'NEW', ...row.slice(3)])
  const r = resolveFromBands(spec('StarterDex'), HEADERS['STARTER_DEX.data']!, display)
  assert.equal(r.shift, -7)

  const renamed = HEADERS['STARTER_DEX.data']!.map((row) => row.map((c) => (c === 'Fought Flag' ? 'Seen Flag' : c)))
  assert.throws(() => resolveFromBands(spec('StarterDex'), renamed, HEADERS['Starter Dex Checklist']!), /could not find the header "Fought Flag"/)
})
...
test('cross-check catches a display that only partly moved', () => {
  ...
  const display = HEADERS['Starter Dex Checklist']!.map((row) => [...row.slice(0, 10), 'NEW', ...row.slice(10)])
  assert.throws(() => resolveFromBands(spec('StarterDex'), HEADERS['STARTER_DEX.data']!, display), /"Classic Wins" is at column/)
})

test('describeResolved is readable', () => {
  const r = resolveFromBands(spec('QuickChecklist'), HEADERS['STARTER_CHECKLIST.data']!, HEADERS['Quick Checklist (migrated)']!)
  assert.equal(describeResolved(r), 'QuickChecklist: data D–K (SHINY … Max IVs) → display H–O (shift +4)')
